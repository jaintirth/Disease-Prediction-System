...
unique_diseases = training_data['prognosis'].unique().tolist()

# core function and helper function
# creating a function to retrieve info regarding the disease
def helper(dis):
    ...
    # Flatten the list to remove the nested structure
    pre = [item for sublist in pre for item in sublist]

    med = medications[medications['Disease'] == dis]['Medication'].tolist()
    if len(med) == 1 and isinstance(med[0], str):
        med = med[0].strip("[]").split(', ')  # Splitting the string into a proper list

    die = diets[diets['Disease'] == dis]['Diet'].tolist()
    # If the extracted value is a single string containing commas, split it into a proper list
    if len(die) == 1 and isinstance(die[0], str):
        die = die[0].strip("[]").split(', ')  # Splitting the string into a proper list
...
def get_symptoms():
    symptoms_list = list(symptoms_dict.keys())  # Get all symptoms
    return {"symptoms": symptoms_list}  # Send as JSON response
